...
import com.hx.newland.cashin.swing.xdl.ByteUtils;
import com.hx.newland.cashin.swing.xdl.DeviceController;
import com.hx.newland.cashin.swing.xdl.DeviceControllerImpl;
import com.hx.newland.cashin.swing.xdl.NewLandUtil;
import com.hx.newland.cashin.swing.xdl.TransferListener;
...
public class SwingCardByXDLAudioActivity extends BaseActivity {
    ...
    private void initUI() {
        ...
        btn_reset.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                /*if(type){
					onSwipCard(SWIPCARD_ME11);
				}else{
					getDeviceInfo(FETCH_DEVICE_INFO);
				}*/
                onSwipCard(SWIPCARD_ME11);
                //Toast.makeText(HaiFuTongSwingCradActivity.this, "刷卡", Toast.LENGTH_SHORT).show();
    ...
    private void initMe11DeviceController(DeviceConnParams params) {
        controller.init(SwingCardByXDLAudioActivity.this, ME11_DRIVER_NAME,
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    type = false;
                    btn_reset.setEnabled(true);
                    appendInteractiveInfoAndShow("获取设备信息失败,请点击重置按钮再刷卡或插卡！");
    ...
    public void DisplayDialog() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(SwingCardByXDLAudioActivity.this, "此设备没有进行绑定，请先到绑定界面进行绑定！", Toast.LENGTH_SHORT).show();
                btn_reset.setEnabled(false);
            }
        });
...
//						/************************************************************/
                        ...
                        appendInteractiveInfoAndShow("检测到IC卡插入,请稍后...");
                    ...
                    appendInteractiveInfoAndShow("交易失败！请点击重置按钮再刷卡或插卡!");
    ...
    private void entryOtherActivity() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                startActivity(new Intent(SwingCardByXDLAudioActivity.this, SignaturePadActivity.class));
                finish();
...
//			appendInteractiveInfoAndShow("交易耗时:" + mTime/1000+"."+mTime%1000 + "s");
        ...
        public void onError(EmvTransController arg0, Exception arg1) {
            ...
            appendInteractiveInfoAndShow("交易失败");
            processingFinished();
        }
...
//					}
    ...
    public void onGetTermList() {
        ...
        MyHttpClient.post(SwingCardByXDLAudioActivity.this,
	...
	/*private void getRateType(JSONArray rateArray){
		
		User.arrayList.clear();
		if(rateArray!=null&&rateArray.length()>0){
			...
			for(int i=0;i<rateArray.length();i++){
				...
				try {
					...
					String rateMaximun=rateArray.getJSONObject(i).getString("rateMaximun");
					String rateNo=rateArray.getJSONObject(i).getString("rateNo");
					String rateDesc=rateArray.getJSONObject(i).getString("rateDesc");
					
					hashMap.put("rateMaximun", rateMaximun);
					hashMap.put("rateNo",rateNo);
					hashMap.put("rateDesc",rateDesc);
					User.arrayList.add(hashMap);
				} catch (JSONException e) {
